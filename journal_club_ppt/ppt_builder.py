...
def add_content_slide(
    ...
    if data.layout == "two-column" and len(data.bullets) > 4:
        left_count = (len(data.bullets) + 1) // 2
        left_bullets = data.bullets[:left_count]
        right_bullets = data.bullets[left_count:]

        content_placeholder.left = Inches(0.6)
        content_placeholder.width = prs.slide_width / 2 - Inches(1.1)
        populate_text_frame(content_placeholder.text_frame, left_bullets, scheme)

        right_left = content_placeholder.left + content_placeholder.width + Inches(0.5)
        right_width = prs.slide_width - right_left - Inches(0.6)
        ...
        populate_text_frame(right_box.text_frame, right_bullets, scheme)
    else:
        populate_text_frame(content_placeholder.text_frame, data.bullets, scheme)
    ...
    if data.callouts and data.layout != "two-column":
        add_callout_cards(prs, slide, data.callouts, scheme)
...
def populate_text_frame(text_frame, bullets, scheme: ColorScheme) -> None:
    """Populate a text frame with bullet content."""
...
def add_callout_cards(
    prs: Presentation,
    slide,
    callouts: list[str],
    scheme: ColorScheme,
) -> None:
    """Render highlighted callout boxes for key numerical data."""

    if not callouts:
        return

    max_items = min(len(callouts), 3)
    card_width = Inches(3.0)
    card_height = Inches(0.9)
    gap = Inches(0.2)
    total_height = max_items * card_height + (max_items - 1) * gap
    top = max(Inches(1.2), prs.slide_height - total_height - Inches(1.0))
    left = prs.slide_width - card_width - Inches(0.6)

    for index, text in enumerate(callouts[:max_items]):
        ...
        box.fill.solid()
        box.fill.fore_color.rgb = scheme.accent
